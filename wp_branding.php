...
function agent_revealid_add_id_column( $columns ) {
$columns['revealid_id'] = 'ID';
return $columns;
}
function agent_revealid_id_column_content( $column, $id ) {
...
add_filter( 'manage_posts_columns', 'agent_revealid_add_id_column', 5 );
add_action( 'manage_posts_custom_column', 'agent_revealid_id_column_content', 5, 2 );
add_filter( 'manage_pages_columns', 'agent_revealid_add_id_column', 5 );
add_action( 'manage_pages_custom_column', 'agent_revealid_id_column_content', 5, 2 );
}
...
add_action( 'admin_head', 'embed_google_font' );

function use_google_font()
{
    $font_face   = vp_option('agnet_branding.logo_font_face');
    $font_weight = vp_option('agnet_branding.vp_get_gwf_weight');
    $font_style  = vp_option('agnet_branding.logo_font_style');
    ...
    <style type="text/css">
    body {
        font-family: <?php echo $font_face; ?> !important;
...
<?php }
add_action( 'admin_head', 'use_google_font' );
