...
class VP_Option_Control_Set
{
	...
	public function save($option_key)
	{
		...
		if(update_option($option_key, $opt))
		{
			...
			$result['message'] = __('Saving successful', 'wp_branding_textdomain');
		...
		else
		{
			...
			if($changed)
			{
				...
				$result['message'] = __('Saving failed', 'wp_branding_textdomain');
			}
			else
			{
				$result['status']  = true;
				$result['code']    = self::SAVE_NOCHANGES;
				$result['message'] = __('No changes made', 'wp_branding_textdomain');
			}
		}
